...
class GFFormDisplay {
	...
	public static function get_form_button( $form_id, $button_input_id, $button, $default_text, $class, $alt, $target_page_number, $onclick = '' ) {
		...
		if ( ! empty( $target_page_number ) ) {
			...
			$onclick = "onclick='if(window[\"gf_submitting_{$form_id}\"]){return false;}  {$set_submitting} {$onclick} {$onclick_submit}' onkeypress='if(window[\"gf_submitting_{$form_id}\"]){return false;}  {$set_submitting} {$onclick} {$onclick_submit}'";
		}
	...
	public static function enqueue_form_scripts( $form, $ajax = false ) {
		...
		if ( self::has_recaptcha_field( $form ) ) {
			wp_enqueue_script( 'gform_recaptcha' );
		}
	...
	public static function print_form_scripts( $form, $ajax ) {
		...
		if ( self::has_recaptcha_field( $form ) && ! wp_script_is( 'gform_recaptcha' ) ) {
			$scripts[] = 'gform_recaptcha';
		}
